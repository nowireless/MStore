...
public class Coll<E> extends CollAbstract<E>
{
	...
	public MStoreUser mstoreUser;
	@Override public MStoreUser getUser() { return this.mstoreUser; }
	
	public Gson getGson()
	{
		return this.mstoreUser.getGson();
	}
	...
	{
		MStore.LOG.error("Database could not load entity. You edited a file manually and made wrong JSON?");
		MStore.LOG.error("Entity: {}", entityId);
		MStore.LOG.error("<k>Collection: {}", this.getName());
		MStore.LOG.error("<k>Error: {}", error);
	}
	...
	protected boolean examineHasLocalAlterFixed(String id, E entity)
	{
		...
		{
			MStore.LOG.fatal("Database examineHasLocalAlter failed convert current entity to JSON tree.");
			MStore.LOG.fatal("Error: {}", e.getMessage());
			MStore.LOG.fatal("Entity: {}", id);
			MStore.LOG.fatal("Collection: {}", this.getName());
	...
	public Coll(String name, Class<E> entityClass, Db db, MStoreUser plugin, boolean creative, boolean lowercasing, Comparator<? super String> idComparator, Comparator<? super E> entityComparator)
	{
	...
	public Coll(String name, Class<E> entityClass, Db db, MStoreUser plugin, boolean creative, boolean lowercasing)
	{
		this(name, entityClass, db, plugin, creative, lowercasing, null, null);
	}
	
	public Coll(String name, Class<E> entityClass, Db db, MStoreUser plugin)
	{
